...
def appointment_list(request,appointment_id):
    if request.user.__str__()!='AnonymousUser':
        ...
        return render(request,'list_any.html',{'title':'Appointment List','list1':appointments,'item_link':'http://tlhclk.pythonanywhere.com/record/make_appointment/','item_title':'Make Appointment'})
    else:
        ...
        if request.user.corporation_id!='':
            ...
            for item in app_list:
                if item.service not in service_dict:
                    ...
                    service_dict[item.service]+=1
            return render(request, 'list_any.html', {'title': 'Appointment List', 'list1': service_dict.items(),'item_link': 'http://tlhclk.pythonanywhere.com/corporation/appointment_list_next/','item_title': 'Appointment List Next'})
    else:
        return redirect('http://tlhclk.pythonanywhere.com/')
